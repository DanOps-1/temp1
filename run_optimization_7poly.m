...
if exist('ga_results.mat', 'file')
    ...
    plot(result_7poly.Plot_X, result_7poly.Phi_Plot, 'r-', 'LineWidth', 2);
    ylabel('角位移 (deg)'); title('槽轮角位移对比');
    legend('五次多项式','七次多项式'); grid on;
    xlim([0 max(result_7poly.Plot_X)+5]);

    subplot(3,1,2);
    plot(result_5poly.Plot_X, result_5poly.Vel_S, 'b--', 'LineWidth', 1.5); hold on;
    plot(result_7poly.Plot_X, result_7poly.Vel_S, 'r-', 'LineWidth', 2);
    ...
    plot(result_7poly.Plot_X, result_7poly.Acc_S, 'r-', 'LineWidth', 2);
    ylabel('角加速度 (rad/s^2)'); xlabel('圆销转角(°)');
    title('槽轮角加速度对比');
    legend('五次多项式','七次多项式'); grid on;
    xlim([0 max(result_7poly.Plot_X)+5]);
